...
Consideremos $X\sim{ Bin (n, \gamma) }$. Entonces su f.m.p. está dada por:
$$P_{x} =
    \begin{pmatrix}
        n\\
        x
...
\begin{eqnarray*}
    ...
    \end{pmatrix}(1-\gamma)^{n-k}\gamma^k}{\begin{pmatrix}
        n\\
        k-1
    \end{pmatrix}(1-\gamma)^{n-k+1}\gamma^{k-1}}\mathbb{I}_{1,2,...,n}(k)=\frac{\frac{n!}{(n-k)!k!}}{\frac{n!}{(n-k+1)(k-1)}}\frac{\gamma}{1-\gamma}=\frac{(n-k+1)!(k-1)!}{(n-k)!k!}\frac{\gamma}{1-\gamma}\\
    &=&\frac{n-k+1}{k}\cdot\frac{\gamma}{1-\gamma}=-\frac{\gamma}{1-\gamma}+\frac{(n+1)\left(\frac{\gamma}{1-\gamma}\right)}{k}=a+\frac{b}{k}{\left\{\begin{array}{lcc}
         \text{Tomando} \\
...
knitr::include_graphics("Imagenes/Cap_4_FrecuenciaAnexo_grafica1.png", error=FALSE)
```
...
```{r echo=FALSE, out.width="50%"}
knitr::include_graphics("Imagenes/BinomNeg.png", error=FALSE)
```
...
```{r echo=FALSE, out.width="50%"}
knitr::include_graphics("Imagenes/Binom.png", error=FALSE)
```
...
```{r echo=FALSE, out.width="50%"}
knitr::include_graphics("Imagenes/Region.png", error=FALSE)
```
...
```{r echo=FALSE, out.width="50%"}
knitr::include_graphics("Imagenes/Cap_4_FrecuenciaAnexo_grafica5.png", error=FALSE)
```
...
En conclusión: $(a,b)\in$ _I_ $\cup$ _III_ $\longrightarrow \exists k \in\mathbb N_{1}$ con $P_{k}<0$; y si $(a,b)\in$ _II_ entonces $\displaystyle\sum_{k=0}^{\infty}P_{k}$ diverge. De tal manera que si $X$ es v.a. con $X\in Tipo(a,b)$, entonces $X$ tiene necesariamente, alguna de las distribuciones _a.1),a.2)_, o _a.3)_$\blacksquare$

_¡La respuesta que has escrito aquí es ejemplar!_

```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("Imagenes/Cap_4_FrecuenciaAnexo_Felicitacion.png", error=FALSE)
```
